  ...
  if (method === "GET" && path === "/auth/me") {
    ...
    if (!profile) {
      const { error: upsertError } = await supabase.from("profiles").upsert({
        id: bearer.authUser.id,
        email: bearer.authUser.email,
        name: bearer.authUser.user_metadata?.name || bearer.authUser.email.split("@")[0],
        phone: bearer.authUser.user_metadata?.phone || null,
        role: "user"
      });
